...
Before going into details it is to be noted that the WGC constraint applies to the microscopic theory where the axion is a primary field.
...
Such a situation occurs if one considers a landscape of chiral fields each of which is charged under a global $U\left(1\right)$ symmetry which is broken by some linear combination of instanton fields.
...
For Dirac-Born-Infeld-inflation CEM does not work.
...
We exhibit coherent enhancement in various settings.
...
In this case, the inflaton would be an axion with an effective axion decay constant which could be significantly different from the primary one.
...
In this work, we propose an alternative mechanism for the enhancement of the effective decay constant associated with inflation.
...
DBI inflation is discussed in \ref{sec:DBI}.
For the DBI case, CEM does not work as there is no analog of $f_e$ since inflation is not controlled by the potential alone but by the full Lagrangian.
...
In this section, we consider inflation in globally supersymmetric models where the slow roll is controlled by a flat potential and CEM can operate.
...
\begin{figure}
  ...
  \caption{\protect\input{figures/supersymmetry.txt}
    The top left panel shows tensor-to-scalar ratio vs scalar spectral index.
...
The results of this process can be seen in Fig.~(\ref{fig:supersymmetry}).
...
For supergravity analysis the saxion can be stabilized by the imposition of spontaneous symmetry breaking conditions~\cite{Nath:1983aw}
\begin{equation}
  D_i W = 0,
  ~~~ i = 1, 2\,.
\end{equation}
...
Simulation results for this potential are similar to the global supersymmetry model and are shown in Fig.~(\ref{fig:supergravity}).
...
As mentioned in section \ref{sec:Introduction} CEM works only for models where the slow roll is governed by flat potentials.
...
\begin{figure}
  ...
  \caption{\protect\input{figures/DBI.txt}
    The top left panel is a plot of $r$ which is the tensor to scalar power spectrum vs the scalar spectral index $n_s$.
...
Simulation results for DBI are shown in Fig.~(\ref{fig:DBI}).
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{
    Top left panel: Evolution of $\rho / \rho_0$, where $\rho_0$ is the density at the beginning of the simulation, as a function of the number of e-foldings until the end of inflation for global supersymmetry case.
    ...
    The figure explains the reason why $r$ can be much larger for DBI relative to models where the slow roll is controlled by flat potentials.
...
However, CEM works for the class of models where the slow roll is governed by potential alone.
...
The inflaton is identified with the pseudo-Nambu-Goldstone boson (pNGB) which is the lightest field in the broken $U\left(1\right)$ symmetry phase.
