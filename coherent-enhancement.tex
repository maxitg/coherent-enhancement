...
For these simulations we set $q = 5$, $G_1 = G_2 = G_3 = 0$, $G_4 = 1$, and we vary $G_5$.
We also set $B = 1$, as it only affects the time scale of inflation, but not the values of $n_s$, $r$, and $f_e$.
We begin by sampling a number of parameter sets $\left(f, G_5, b_{-, 0}, N_\text{pivot}\right)$ from a distribution described in the caption of Fig.~\ref{fig:supersymmetry}.
...
Finally, if at least $N_\text{pivot}$ e-foldings are produced, we compute (COMPUTE HOW?) the tensor-to-scalar ratio $r$, and the scalar spectral index $n_s$ at horizon exit (i.e., $N_\text{pivot}$ e-foldings before the end of inflation), and check if they are consistent with experimental constraints~\cite{Ade:2015lrj}.
If so, we evaluate $f_e$ at horizon exit using Eq.~\ref{eq:feFromPotential}.
The results of this process can be seen on Fig.~\ref{fig:supersymmetry}.

One can see that even though true decay constant $f$ is below $M_\text{P}$, the effective $f_e$ always satisfies constraint \ref{eq:feExperimentalConstraint}.
Note, fine tuning of $G_5$ is required to achieve coherent enhancement and experimentally-consistent inflation.

ADD DISTINCTION BETWEEN $f_e$, and $f_{e, \text{dynamic}}$. ALSO, CHECK $M_\text{P}$ IS USED EVERYWHERE IT SHOULD BE USED.

INCLUDE PLOTS COMPARING $f_e$ with $f_{e, \text{dynamic}}$.

\begin{figure}
  \centering
  \begin{subfigure}{0.45 \textwidth}
    \includegraphics[width = \textwidth]{figures/supersymmetryInputs.pdf}
  \end{subfigure}
  \begin{subfigure}{0.45 \textwidth}
  ...
  \end{subfigure}
  \caption{Figure this fgure.} \label{fig:supersymmetry}
\end{figure}
