...
\begin{equation} \label{eq:feFromDynamicSlowRollParameters}
  f_{eH} = \frac{1}{\sqrt{\eta_H - 2 \epsilon_H}}\,.
\end{equation}
...
Finally, if at least $N_\text{pivot}$ e-foldings are produced, we compute the tensor-to-scalar ratio $r$, and the scalar spectral index $n_s$ at horizon exit (i.e., $N_\text{pivot}$ e-foldings before the end of inflation) using Eqs.~(\ref{eq:slowRollParametersDynamic}), (\ref{eq:slowRollParametersDynamicFromStatic}) and (\ref{eq:observablesSlowRoll}), and check if they are consistent with experimental constraints~\cite{Ade:2015lrj}.
If so, we evaluate $f_e$ and $f_{eH}$ at horizon exit using Eqs.~(\ref{eq:feFromPotential}) and (\ref{eq:feFromDynamicSlowRollParameters}).
...
ALSO, CHECK $M_\text{P}$ IS USED EVERYWHERE IT SHOULD BE USED. CHECK PARENTHESES AFTER EQUATIONS REFERNECE.
