...
Further, one can define a more general effective axion decay constant in terms of inflation density rather than just the potential which valid for models where inflation is inflation is controlled potential and and valid for DBI-flation and more generally for k-flation.
...
Here we propose a new mechanism, i.e., the Coherent Enhancement Mechanism (CEM), where slow roll is governed a flat potential which allows the effective decay constant $f_e \gg M_{Pl}$ while the primary decay constant $f < M_{Pl}$ consistent with WGC~\cite{Nath:2017ihp}.
...
In section \ref{sec:r}, we discuss the reason why the ratio $r$ of tensor-to-scalar power spectra is $O\left(10^{-4}\right)$ for single field inflation for models with a flat potential while it can be much larger in DBI-flation up to $O\left(0.1\right)$.
...
In its simplest form the Weak Gravity Conjecture considers the coupling of an abelian gauge field with gravity and states that this system must contain a particle of charge $q$ and mass $m$ so that~\cite{ArkaniHamed:2006dz} $\frac{q}{m} > \frac{1}{M_{Pl}}$, where $M_{Pl}$ is the reduce Planck mass defined by $M_{Pl} = \left(8 \pi G_N\right)^{-1 / 2}$ and $G_N$ is Newton's constant.
...
We set initial field velocity $\dot b_{-, \text{init}} = 0$, and we take $N_\text{subhorizon} \geq 5$ where $N_\text{subhorizon} = N_\text{total} - N_\text{pivot}$ to ensure that the results of the analysis are not affected by initial conditions.
