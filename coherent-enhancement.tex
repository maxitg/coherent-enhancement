...
\begin{equation}
  K = - M_\text{P}^2 \log\left(\frac{S + \bar S}{M_\text{P}}\right)
      - 2 M_\text{P}^2 \log\left(\frac{\mathcal{V} + \alpha}{M_\text{P}^{3 / 2}}\right)
      + K_{cs}\left(U, \bar U\right)\,.
\end{equation}
Here $S$ is the dilation, $U$ are the complex moduli.
For moduli stabilization we need $\alpha$-correction to the K\"ahler potential where $\alpha = \frac{1}{2} \xi S_1^{3 / 2}$, $\xi = -\zeta\left(3\right) \chi / \left(16 \pi^3\right)$, $\zeta$ is the Riemann zeta function, and $\chi = 2 \left(h^{1, 1} - h^{1, 2}\right)$ is the topological Euler characteristic of the compactified manifold $X$.
...
\end{equation}
where we have $T_i = \tau_i + i \theta_i$, $i = b, s$.
In the analysis we will assume that $S$ and $U$ have been stabilized at large scales so that $S = \bar{S} = S_1$ and $K_{cs} = 0$ and thus these will play no role in the inflation analysis.
We will thus focus on the stabilization of the remaining moduli.
Here we assume a superpotential of the form
\begin{equation}
  W = W_0
    + A_s e^{-\gamma_s T_s / M_\text{P}}
    + \sum_{n = 1}^q A_{b, n} e^{-\gamma_{b, n} T_b / M_\text{P}}\,.
\end{equation}
...
\begin{equation}
  \begin{aligned}
    V = &M_\text{P}^4 \left(
        C
      + C_s \cos\left(\frac{\gamma_s \theta_s}{M_\text{P}}\right)
      + \sum_{n = 1}^q C_{b, n} \cos\left(\frac{\gamma_{b, n} \theta_b}{M_\text{P}}\right)\right.
      + \sum_{n = 1}^q C_{sb, n} \cos\left(
           \frac{\gamma_s \theta_s - \gamma_{b, m} \theta_b}{M_\text{P}}
         \right)\\
      &~~~~~~ \left.{}
      + \sum_{n = 1}^q \sum_{m = n + 1}^q C_{bb, n m} \cos\left(
           \frac{\left(\gamma_{b, n} - \gamma_{b, m}\right) \theta_b}{M_\text{P}}
         \right)
    \right)\,,
  \end{aligned}
\end{equation}
...
\begin{equation}
    \begin{aligned}
    C &= \frac{1}{\mathcal{D}} \left(
        \frac{3}{2} \frac{\alpha}{M_\text{P}^{3 / 2}} \frac{W_0^2}{M_\text{P}^6}
      \right.\\ &~~~ \left.{}
      + e^{-2 \gamma_s \tau_s / M_\text{P}}
        \frac{A_s^2}{M_\text{P}^6}
        \right.\\ &~~~~~~ \left.{} \times
        \frac{
            4 \left(
                  \gamma_s \tau_s / M_\text{P}
                + \left(1 / 3\right) \gamma_s^2 \left(\tau_s^2 + 2 \tau_s^{1 / 2} \tau_b^{3 / 2} - \alpha \tau_s^{1 / 2} / \eta\right) / M_\text{P}^2\right)
              \left(\mathcal{V} + \alpha\right)
            + \left(3 / 2\right) \alpha}{
            M_\text{P}^{3 / 2}}
      \right.\\ &~~~ \left.{}
      + \sum_{n = 1}^q
          e^{-2 \gamma_{b, n} \tau_b / M_\text{P}}
          \frac{A_{b, n}^2}{M_\text{P}^6}
          \right.\\ &~~~~~~ \left.{} \times
          \frac{
            4 \left(
                  \gamma_{b, n} \tau_b / M_\text{P}
                + \left(1 / 3\right) \gamma_{b, n}^2 \left(\tau_b^2 + 2 \tau_b^{1 / 2} \tau_s^{3 / 2} + \alpha \tau_b^{1 / 2} / \eta\right) / M_\text{P}^2\right)
              \left(\mathcal{V} + \alpha\right)
            + \left(3 / 2\right) \alpha}{
            M_\text{P}^{3 / 2}}
    \right)\,,\\
    C_s &=
      \frac{1}{\mathcal{D}} \frac{
        4 \gamma_s \left(\tau_s / M_\text{P}\right)
            \left(\mathcal{V} + \alpha\right)
          + 3 \alpha}{M_\text{P}^{3 / 2}}
      e^{-\gamma_s \tau_s / M_\text{P}}
      \frac{W_0 A_s}{M_\text{P}^6}\,,\\
    C_{b, n} &=
      \frac{1}{\mathcal{D}} \frac{
        4 \gamma_{b, n} \left(\tau_b / M_\text{P}\right)
            \left(\mathcal{V} + \alpha\right)
          + 3 \alpha}{M_\text{P}^{3 / 2}}
      e^{-\gamma_{b, n} \tau_b / M_\text{P}}
      \frac{W_0 A_{b, n}}{M_\text{P}^6}\,,\\
    C_{sb, n} &= \frac{1}{\mathcal{D}}
      \frac{
          4 \left(
                \left(\gamma_s \tau_s + \gamma_{b, n} \tau_b\right) / M_\text{P}
              + 2 \gamma_s \gamma_{b, n} \tau_s \tau_b / M_\text{P}^2\right)
            \left(\mathcal{V} + \alpha\right)
        + 3 \alpha}{
        M_\text{P}^{3 / 2}}
      e^{-\left(\gamma_s \tau_s + \gamma_{b, n} \tau_b\right) / M_\text{P}}
      \frac{A_s A_{b, n}}{M_\text{P}^6}\,,\\
    C_{bb, n m} &= \frac{1}{\mathcal{D}}
      e^{-\left(\gamma_{b, n} + \gamma_{b, m}\right) \tau_b / M_\text{P}}
      \frac{A_{b, n} A_{b, m}}{M_\text{P}^6}
      \\ & {} \times
      \frac{
        4 \left(
              \left(\gamma_{b, n} + \gamma_{b, m}\right) \tau_b / M_\text{P}
            + \left(2 / 3\right) \gamma_{b, n} \gamma_{b, m} \left(\tau_b^2 + 2 \tau_s^{3 / 2} \tau_b^{1 / 2} + \alpha \tau_b^{1 / 2} / \eta\right) / M_\text{P}^2\right)
          \left(\mathcal{V} + \alpha\right)
        + 3 \alpha}{
        M_\text{P}^{3 / 2}}\,,
    \end{aligned}
...
and
\begin{equation}
  \mathcal{D} = \frac{S_1}{M_\text{P}} \left(\frac{\mathcal{V} + \alpha}{M_\text{P}^{3 / 2}}\right)^2 \frac{2 \mathcal{V} - \alpha}{M_\text{P}^{3 / 2}}\,.
\end{equation}

We then set $INSERT FIXED PARAMETERS$, and vary $INSERT VARYING PARAMETERS$ as described in the caption of Fig.~(\ref{fig:LVS}).
For each set of input parameters, all four fields are numerically stabilized, and simultaneously we choose $A_s$ such that potential vanishes at the stability point.
Note that we can take an approximation of a single-field inflation in $\theta_b$ in this case as for all parameter sets considered
\begin{equation}
  EQUATION COMPARING MASS OF \theta_b TO OTHER FIELDS
\end{equation}
