...
\title{
 \rr{Coherent Enhancement of the Axion Decay Constant in Inflation and the Weak Gravity Conjecture}
...
However, a super \rr{Planckian} axion decay constant is disfavored in quantum gravity and in \rr{strings, and by the weak gravity 
conjecture.}
...
\rr{The} current data on $n_s$ and $r$ constrains the effective axion decay constant so that $4.9 \leq f_e / M_\text{P} \leq 10.0$ at $95\%$ CL.
...
We illustrate the coherent enhancement in several settings: globally supersymmetric models, supergravity models, and in string setting 
\rr{such as the Large Volume Scenario.}
...
In each case, all the moduli are stabilized and the inflationary model consistent with astrophysical observations with $f_e > M_\text{P}$ and the true axion decay constant $f < M_\text{P}$ \rr{consistent with the weak gravity conjecture}.
\newpage
...
Thus in section \ref{sec:Supersymmetry} we discuss this mechanism in globally supersymmetric models, in section \ref{sec:Supergravity} for supergravity models, 
\rr{and in section \ref{sec:LVS} for a model inspired by the Large Volume Scenario.
In section \ref{sec:DBI}, we discuss the Dirac-Born-Infeld case, in which inflation is not controlled by the potential alone but by the 
full Lagrangian.  However, an effective decay constant $f_{eH}$ can still be defined based on inflation dynamics.
In section \ref{sec:WGC} we discuss the weak gravity conjecture and axion inflation.}
...
\rr{An interesting} suggestion to realize $f < M_\text{P}$ is to use two axions and the alignment mechanism to achieve a flat direction~\cite{Long:2014dta}.
...
\end{equation}
Then, one \rr{imposes}  a constraint for a flat direction:
\begin{equation}
  \frac{f_1}{f_2} = \frac{f_3}{f_4}\,,
  \label{alignment-constraint}
\end{equation}
\rr{and considers small corrections to that constraint to produce a light inflaton.
Here, we discuss a  coherent enhancement mechanism that can lead to an effective axion decay constant 
that controls inflation and 
is much larger than the true decay constant.
Further, exhibit this phenomenon in models based on supersymmetry and supergravity, and in string inspired models}.
...
Next, we discuss \rr{the coherent enhancement mechanism arising from}
 a superposition of cosine functions and show how the effective axion decay constant becomes super-Planckian even though the true decay constant is sub-planckian.
...
\rr{We will show using numerical simulations that \rr{show} $f_e \approx f_{eH}$ for the cases of global supersymmetry, supergravity, and
 LVS. The DBI inflation  is discussed in \ref{sec:DBI}. For the DBI case  there is no analogue of $f_e$ since inflation in not controlled
 by the potential alone but  by the full Lagrangian. 
 However, for DBI one may still define an $f_{eH}$ as given by  Eq.~(\ref{eq:feFromDynamicSlowRollParameters})
 which may be compared to the true axion decay constant that enters in the DBI lagrangian. }
...
which violates the shift symmetry.
 \rr{Here we note  that a similar procedure of using several non-perturbative terms
to produce inflation by adjustment of parameters in the non-perturbative terms is used in the 
so-called `racetrack' models (see, e.g.,~\cite{BlancoPillado:2004ns,Lalak:2005hr,Greene:2005rn,BlancoPillado:2006he})}.
...
An alternate way to describe the constraint is to define the parameter 
$z= \frac{qM_P}{m}$ and in this case the WGC constraint implies  $z>1$. The WGC constraint  can be extended to 
the case where there are  several abelian gauge groups~\cite{Cheung:2014vva}
$\prod_{\alpha=1}^{N}U(1)_{\alpha}$ with each $U(1)_{\alpha}$ having particles of charges $q_{i\alpha}$ and masses
$m_i$.  In this case the allowed parameter space of masses and charges must lie outside a convex
hull which contains the unit ball whose boundary corresponds to extremal an black  hole
where  for the extremal black hole $z_{BH}= \frac{QM_P}{M}=1$ and  where $Q$ is the black hole charge and $M$ its mass.
...
one cannot let the charge $q_i$ become arbitrarily small since in that case in the limit one will have
a continuous global symmetry which is forbidden by strings.

So far the analysis concerns just the abelian gauge theories coupling with gravity. However, there are
 implications also for the axions and axionic inflation.  The constraints of the WGC for axions are more
 indirectly arrived at and there is less unanimity regarding what their implications are. 
 Thus even before WGC, 
...
For the simplest case of a single axion field 
 a decay constant in the trans-Planckian region appears improbable. For the most general case of multiple axions
 with kinetic mixings and more general form for arguments in the cosines, the analysis is somewhat ambiguous. 
 There is a significant literature associated with this topic.  
 \cite{Rudelius:2015xta,Rudelius:2014wla,Bachlechner:2014gfa,Choi:2014rja, 
 Hebecker:2015rya,Conlon:2016aea,Montero:2015ofa,Junghans:2015hba}.


In this work we assume that WGC holds in the axion sector and implies that $f_i <M_{Pl}$ for any number of 
canonically normalized axion fields. However, we show that two (or more) axion fields which transform under 
a $U(1)$ global symmetry  along with non-perturbative symmetry breaking terms can enter in coherent enhancement of
the effective decay constant during the inflationary period.
 It differs from  
the alignment mechanism in that it does not rely on Eq.(\ref{alignment-constraint})
 to create a flat 
direction. A massless inflaton is generated by an underlying $U(1)$ symmetry which is then 
broken by instanton like terms producing a potential for the inflaton field.
The enhancement occurs from a superposition
of several cosine terms, as can be seen from Eq.~(\ref{eq:supersymmetry:Vslow})
for the global supersymmetry case,
from Eq.(\ref{eq:supergravity:Vslow3})
for the supergravity case, and 
from Eq.(\ref{LVS-case}) 
for the LVS case.  In section 3, we show that for any model with
canonical kinetic energy and a potential which is not velocity dependent, 
that an enhancement of the axion decay constant can be achieved 
by superposition of several terms which break the $U(1)$
global symmetry. The enhancement arises from constructive and destructive 
interference among the various terms as shown in Eq.(\ref{eq:feForCosineSum}).
In section 3  we also show that the
current experimental data requires the effective decay constant to lie in the
trans-Planckian region $4.6< f_e<11$. We construct explicit models which obey this 
experimental constraint using the coherent enhancement mechanism
while the true decay constant lies in the
sub-Planckian region consistent with the WGC constraint. All the moduli are stabilized
in the analysis.
\\


In addition to the above we have investigated the supersymmetric DBI model.
Here the normal analysis using the potential alone does not work  and one
must utilize the full Lagrangian. 
In this case we show that consistent inflation can occur
with the true decay constant which is sub-Planckian and further we 
provide a workable definition of the effective decay constant in terms 
of the inflation parameters $n_s$ and $r$.  The WGC constraints
in this case are unknown, and it is of interest to investigate 
 them for DBI inflation.
We note that our analysis  including LVS and
DBI are in the spirit of an effective theory. The analyses in these models use 
 sub-Planckian decay constants, all the moduli are fully stabilized 
  and generate inflation consistent with the current
cosmological data. It is of interest to show that UV completion of these 
models  could be constructed and lie 
...
\rr{The mechanism utilizes the symmetry breaking part which generates the axion potential, 
is quite general and applies naturally to inflaton potentials arising in models based in supersymmetry, supergravity and strings.
The analysis is extended to inflation in supersymmetric  Dirac-Born-Infeld model which, however, is more complex since
inflation is governed not just by the potential but the Lagrangian. 
...
as exhibited by   Eq.(\ref{fe-spectral-indices}). The analysis presented in this work shows that in all the cases considered 
axion inflation consistent with the experimental data can be accomplished with the true axion decay constant in the sub-Planckian
...
\bibitem{ArkaniHamed:2006dz} 
  ...
  %484 citations counted in INSPIRE as of 10 Apr 2019
...
\bibitem{Cheung:2014vva} 
  ...
  %%CITATION = doi:10.1103/PhysRevLett.113.051601;%%
  %94 citations counted in INSPIRE as of 10 Apr 2019
...
\bibitem{Banks:2003sx} 
  ...
  %266 citations counted in INSPIRE as of 10 Apr 2019
...
\bibitem{BlancoPillado:2004ns} 
  ...
  %237 citations counted in INSPIRE as of 30 Dec 2017
...
\bibitem{Lalak:2005hr} 
  ...
  [hep-th/0503178].
  %%CITATION = doi:10.1016/j.nuclphysb.2006.06.041;%%
  %54 citations counted in INSPIRE as of 30 Dec 2017
%\cite{Greene:2005rn}
\bibitem{Greene:2005rn} 
  B.~Greene and A.~Weltman,
  ``An Effect of alpha' corrections on {\bf racetrack inflation},''
  JHEP { 0603}, 035 (2006)
  doi:10.1088/1126-6708/2006/03/035
  [hep-th/0512135].
  %%CITATION = doi:10.1088/1126-6708/2006/03/035;%%
  %13 citations counted in INSPIRE as of 30 Dec 2017
...
\bibitem{BlancoPillado:2006he} 
  ...
  %133 citations counted in INSPIRE as of 30 Dec 2017
...
\bibitem{Brown:2015iha} 
  ...
  %%CITATION = doi:10.1007/JHEP10(2015)023;%%
  %137 citations counted in INSPIRE as of 10 Apr 2019

%\cite{Heidenreich:2015wga}
\bibitem{Heidenreich:2015wga} 
  ...
  %112 citations counted in INSPIRE as of 18 Apr 2019
%\cite{Rudelius:2015xta}
\bibitem{Rudelius:2015xta} 
  ...
  %116 citations counted in INSPIRE as of 10 Apr 2019


%\cite{Rudelius:2014wla}
\bibitem{Rudelius:2014wla} 
  ...
  %%CITATION = doi:10.1088/1475-7516/2015/04/049;%%
  %76 citations counted in INSPIRE as of 10 Apr 2019


%\cite{Bachlechner:2014gfa}
\bibitem{Bachlechner:2014gfa} 
  ...
  %%CITATION = doi:10.1007/JHEP12(2015)042;%%
  %56 citations counted in INSPIRE as of 10 Apr 2019
...
\bibitem{Choi:2014rja} 
  ...
  %%CITATION = doi:10.1103/PhysRevD.90.023545;%%
  %141 citations counted in INSPIRE as of 19 Apr 2019




%\cite{Hebecker:2015rya}
\bibitem{Hebecker:2015rya} 
  ...
  %%CITATION = doi:10.1016/j.physletb.2015.07.026;%%
  %91 citations counted in INSPIRE as of 10 Apr 2019
...
\bibitem{Conlon:2016aea} 
  ...
  %%CITATION = doi:10.1007/JHEP04(2016)085;%%
  %25 citations counted in INSPIRE as of 10 Apr 2019


%\cite{Montero:2015ofa}
\bibitem{Montero:2015ofa} 
  ...
  %%CITATION = doi:10.1007/JHEP08(2015)032;%%
  %109 citations counted in INSPIRE as of 10 Apr 2019
%\cite{Junghans:2015hba}
\bibitem{Junghans:2015hba} 
  ...
  %%CITATION = doi:10.1007/JHEP02(2016)128;%%
  %56 citations counted in INSPIRE as of 19 Apr 2019
